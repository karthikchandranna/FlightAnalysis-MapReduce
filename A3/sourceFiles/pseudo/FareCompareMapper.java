/**
 *
 * Created by Karthik and Sujith on 1/29/16.
 */
...
public class FareCompareMapper extends Mapper<LongWritable, Text, Text, FloatWritable> {
    @Override
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if (columns.length == 110 && Double.parseDouble(columns[109]) < 100000 && sanityTest(columns)) {
            //key would be airline code and month
            String carrierInfo = columns[8]+columns[2];

            float priceDollars = Float.parseFloat(columns[109]);

            // price is in cents
            context.write(new Text(carrierInfo), new FloatWritable(priceDollars));
        }
    }
